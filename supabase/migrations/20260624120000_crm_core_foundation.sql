...
create table public.crm_company_members (
  id uuid primary key default gen_random_uuid(),
...
create index crm_company_members_company_id_idx on public.crm_company_members(company_id);
create index crm_company_members_user_id_idx on public.crm_company_members(user_id);
...
create index crm_clients_company_id_idx on public.crm_clients(company_id);
...
create index crm_contacts_client_id_idx on public.crm_contacts(client_id);

create table public.crm_dossiers (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.crm_companies(id) on delete cascade,
...
create index crm_dossiers_company_id_idx on public.crm_dossiers(company_id);
create index crm_dossiers_client_id_idx on public.crm_dossiers(client_id);
...
create index crm_notes_dossier_id_idx on public.crm_notes(dossier_id);
...
create index crm_tasks_dossier_id_idx on public.crm_tasks(dossier_id);
...
create index crm_documents_client_id_idx on public.crm_documents(client_id);
create index crm_documents_dossier_id_idx on public.crm_documents(dossier_id);
...
create index crm_activity_log_company_id_idx on public.crm_activity_log(company_id);
create index crm_activity_log_profile_id_idx on public.crm_activity_log(profile_id);
...
-- ---------------------------------------------------------------------------
create trigger crm_companies_set_updated_at before update on public.crm_companies for each row execute function public.set_updated_at();
create trigger crm_clients_set_updated_at before update on public.crm_clients for each row execute function public.set_updated_at();
...
create trigger crm_dossiers_set_updated_at before update on public.crm_dossiers for each row execute function public.set_updated_at();
...
-- crm_companies: visible to members; created by owner; managed by owner.
...
-- crm_company_members: members can read; owner manages.
...
-- crm records: full access for any member of the owning company.
...
-- crm_activity_log: members read; insert only as self into accessible company.
...
-- user_preferences: strictly own row.
